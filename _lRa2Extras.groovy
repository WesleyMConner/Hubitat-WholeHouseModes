...
void parseRa2IntegRpt() {
  ...
  Map ra2Timeclock = [:]
  Map ra2GreenMode = [:]
  String[] integRpt = settings.ra2IntegReport.split('\n')
  Integer row = 0
  while (row < integRpt.size()) {
    // Cleanup Lutron's sloppy CSV
    row = row.trim()?.replaceAll(', ', ',')
    String rowData = integReport[row]
    if (row == 0 && rowData != 'RadioRA 2 Integration Report') {
      logError('parseRa2IntegRpt', "Unexpected data at row #0: >${rowData}<")
    } else if (rowData == 'Device Room,Device Location,Device name,Model,ID,Component,Component Number,Name') {
      // Process Consecutive Kpad Data
      while ({
        // Tactically, toss non-empty rows.
        rowData = integReport[++row]
        rowData != '' // !x.isFinished()
      }()) continue
    } else if (rowData == '') {
      ...
      // Processing Timeclock Data
    } else if (rowData == '') {
      // Processing Green Button Data
    }
