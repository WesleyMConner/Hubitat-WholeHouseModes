...
void pbsgButtonOnCallback(String pbsgLabel, String mode) {
  if (pbsgLabel == '_Mpbsg') {
    // - Mode PBSG calls this method to reflect a Hubitat mode change.
...
Map WhaPage() {
  return dynamicPage(
    ...
    state.remove('MODE_PBSG_LABEL')  // state.MPBSG_LABEL = '_MPbsg'
    state.remove('modeButtonMap')
    state.remove('MPBSG_LABEL')
    app.updateLabel('WHA')
    state.remove('MODES')  //state.MODES = getLocation().getModes().collect{ it.name }
    logWarn('WhaPage', 'Adding mode PBSG "_MPbsg"')
